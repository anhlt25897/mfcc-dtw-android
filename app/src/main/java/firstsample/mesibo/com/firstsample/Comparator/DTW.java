package firstsample.mesibo.com.firstsample.Comparator;
...
public final class DTW {
    ...
    //region PUBLIC UTILS
    public DTW.Result process(List<MFCCVector> pSample, List<MFCCVector> pTemplate) {
        return compute(pSample, pTemplate);
    }
    //endregion

    //region PRIVATE UTILS
    private DTW.Result compute(List<MFCCVector> pSample, List<MFCCVector> pTemplate) {
        ...
        for (i = 0; i < lN; i++) {
            // Fetch the Sample.
            MFCCVector lSample = pSample.get(i);
     ...
     */
    private double getDistanceBetween(MFCCVector p1, MFCCVector p2) {
        double sqSum = 0.0;
        sqSum += Math.pow(p1.energy - p2.energy, 2.0);
        for (int x = 0; x < p1.props.length; x++) {
            sqSum += Math.pow(p1.props[x] - p2.props[x], 2.0);
        }
